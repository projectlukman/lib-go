...
func (c *Client) MongoConnect() (mc *mongo.Client, err error) {
	...
	ctx, ctxCancel := context.WithTimeout(context.Background(), c.ConnectTimeout)
	defer ctxCancel()
	// otelMon := otelmongo.NewMonitor(c.AppName)
	...
	ctx, ctxCancel = context.WithTimeout(context.Background(), c.PingTimeout)
	defer ctxCancel()
